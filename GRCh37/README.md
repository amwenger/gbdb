...
```
LTMPDIR=$(mktemp -d)
rsync -avzP rsync://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/genomicSuperDups.txt.gz $LTMPDIR
...
```
LTMPDIR=$(mktemp -d)
rsync -avzP rsync://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/chainSelf.txt.gz $LTMPDIR
rsync -avzP rsync://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/chainSelfLink.txt.gz $LTMPDIR
...
```
LTMPDIR=$(mktemp -d)
rsync -avzP rsync://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/rmsk.txt.gz $LTMPDIR
...
mysql --user=hgtestuser --host=genome-testdb.cse.ucsc.edu -A \
    -Ne "SELECT chrom, chromStart, chromEnd FROM hg19.windowmaskerSdust" |
    sort -k1,1 -k2,2g | bgzip -c > annotation/windowmaskerSdust.bed.gz
...
```
LTMPDIR=$(mktemp -d)
rsync -avzP rsync://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/simpleRepeat.txt.gz $LTMPDIR
...
```
LTMPDIR=$(mktemp -d)
rsync -avzP rsync://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/gap.txt.gz $LTMPDIR
...
```
LTMPDIR=$(mktemp -d)
rsync -avzP rsync://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/refGene.txt.gz $LTMPDIR
...
```
LTMPDIR=$(mktemp -d)
rsync -avzP rsync://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/knownGene.txt.gz $LTMPDIR
zcat $LTMPDIR/knownGene.txt.gz | awk '{ print ".\t" $0; }' |
    awk '{ split($10,ES,","); split($11,EE,","); for(i=1;i<=$9;i++) { s=ES[i]; e=EE[i]; if($8>$7) { print $3"\t"s"\t"e"\t"$2;  } } }' |
    sort -k1,1 -k2,2g | bgzip -c > annotation/knownGene.exon.bed.gz
tabix -f annotation/knownGene.exon.bed.gz
zcat $LTMPDIR/knownGene.txt.gz | awk '{ print ".\t" $0; }' |
    awk '{ split($10,ES,","); split($11,EE,","); for(i=1;i<=$9;i++) { s=ES[i]>$7?ES[i]:$7; e=EE[i]<$8?EE[i]:$8; if(e>s) { print $3"\t"s"\t"e"\t"$2;  } } }' |
    sort -k1,1 -k2,2g | bgzip -c > annotation/knownGene.cds.bed.gz
tabix -f annotation/knownGene.cds.bed.gz
zcat $LTMPDIR/knownGene.txt.gz | awk '{ print ".\t" $0; }' |
    awk '{ split($10,ES,","); split($11,EE,","); for(i=1;i<=$9;i++) { if(ES[i]<$7) { s=ES[i]; e=EE[i]<$7?EE[i]:$7; { print $3"\t"s"\t"e"\t"$2; } } if(EE[i]>$8) { s=ES[i]>$8?ES[i]:$8; e=EE[i]; print $3"\t"s"\t"e"\t"$2; } } }' |
    sort -k1,1 -k2,2g | bgzip -c > annotation/knownGene.utr.bed.gz
tabix -f annotation/knownGene.utr.bed.gz
rm -rf $LTMPDIR && unset LTMPDIR
```
...
```
LTMPDIR=$(mktemp -d)
rsync -avzP rsync://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/ensGene.txt.gz $LTMPDIR
zcat $LTMPDIR/ensGene.txt.gz |
    awk '{ split($10,ES,","); split($11,EE,","); for(i=1;i<=$9;i++) { s=ES[i]; e=EE[i]; if($8>$7) { print $3"\t"s"\t"e"\t"$2;  } } }' |
    sort -k1,1 -k2,2g | bgzip -c > annotation/ensGene.exon.bed.gz
tabix -f annotation/ensGene.exon.bed.gz
zcat $LTMPDIR/ensGene.txt.gz |
    awk '{ split($10,ES,","); split($11,EE,","); for(i=1;i<=$9;i++) { s=ES[i]>$7?ES[i]:$7; e=EE[i]<$8?EE[i]:$8; if(e>s) { print $3"\t"s"\t"e"\t"$2;  } } }' |
    sort -k1,1 -k2,2g | bgzip -c > annotation/ensGene.cds.bed.gz
tabix -f annotation/ensGene.cds.bed.gz
zcat $LTMPDIR/ensGene.txt.gz |
    awk '{ split($10,ES,","); split($11,EE,","); for(i=1;i<=$9;i++) { if(ES[i]<$7) { s=ES[i]; e=EE[i]<$7?EE[i]:$7; { print $3"\t"s"\t"e"\t"$2; } } if(EE[i]>$8) { s=ES[i]>$8?ES[i]:$8; e=EE[i]; print $3"\t"s"\t"e"\t"$2; } } }' |
    sort -k1,1 -k2,2g | bgzip -c > annotation/ensGene.utr.bed.gz
tabix -f annotation/ensGene.utr.bed.gz
rm -rf $LTMPDIR && unset LTMPDIR
```
...
```
mysql --user=hgtestuser --host=genome-testdb.cse.ucsc.edu -A \
    -Ne "SELECT chrom, chromStart, chromEnd, name FROM hg19.par" |
...
```
# Download the UCSC genes tracks, which has gene models named "abParts" that cover
# the IG regions of interest.
LTMPDIR=$(mktemp -d)
rsync -avzP rsync://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/kgXref.txt.gz $LTMPDIR
zcat $LTMPDIR/kgXref.txt.gz | awk -F'\t' '($5=="abParts") { print $1; }' | sort > $LTMPDIR/abPartsGenes.txt

rsync -avzP rsync://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/knownGene.txt.gz $LTMPDIR
zcat $LTMPDIR/knownGene.txt.gz | sort -k1,1 | join -t $'\t' - $LTMPDIR/abPartsGenes.txt |
...
```
LTMPDIR=$(mktemp -d)
curl -s ftp://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh37/clinvar.vcf.gz > $LTMPDIR/clinvar.vcf.gz
